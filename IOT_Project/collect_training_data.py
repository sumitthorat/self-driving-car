...
class CollectTrainingData(object):
    ...
    def __init__(self):
        ...
        self.server_socket.bind(('0.0.0.0', 8011))
                    ...
                    for event in pygame.event.get():
                        if event.type == KEYDOWN:
                            ...
                            if key_input[pygame.K_w] and key_input[pygame.K_d]:
                                print("Forward Right")
                                cv2.imwrite('training_images_raw/fwd_right/' + time.strftime("%Y%m%d-%H%M%S") + '.jpg', roi)
                                ...
                                print("Forward Left")
                                cv2.imwrite('training_images_raw/fwd_left/' + time.strftime("%Y%m%d-%H%M%S") + '.jpg',
                                            roi)
                                image_array = np.vstack((image_array, temp_array))
                            ...
                            if key_input[pygame.K_w]:
                                print("Forward")
                                cv2.imwrite('training_images_raw/fwd/' + time.strftime("%Y%m%d-%H%M%S") + '.jpg',
                                            roi)
                                saved_frame += 1
                                image_array = np.vstack((image_array, temp_array))
